<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('trasnsactions', function (Blueprint $table) {
            $table->id();
            $table->foreignId('id_lokasi') -> references('id') ->on('locations') ->onDelete('cascade') ->onUpdate('cascade') ;
            $table->string('no_tiket') ;
            $table->string('no_polisi') ;
            $table->foreignId('id_jenis') -> references('id') ->on('vehicle_types') ->onDelete('cascade') ->onUpdate('cascade') ;
            $table->datetime('masuk');
            $table->datetime('keluar')->nullable();
            $table->integer('perjam_pertama')->default(0);
            $table->integer('perjam_berikutnya')->default(0);
            $table->integer('max_perhari')->default(0);
            $table->integer('total_jam')->default(0);
            $table->integer('total_bayar')->default(0);
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('trasnsactions');
    }
};
